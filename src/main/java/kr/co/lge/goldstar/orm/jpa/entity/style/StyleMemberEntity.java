 ...
 */
@Entity(name="styleMember")
@Table(name="tb_style_member")
@Data
public class StyleMemberEntity implements Serializable {
    
    /**
     * 
     */
    public StyleMemberEntity() {
        this.id = new StyleMemberId();
    }
    
    private static final long serialVersionUID = 5623830977450431211L;

    @EmbeddedId
    private StyleMemberId id;
    ...
    private StylePartEntity part;

    @JsonIgnore
    @MapsId("answerSn")
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "answer_sn", insertable = false, updatable = false)
    private StyleAnswerEntity answer;
}
