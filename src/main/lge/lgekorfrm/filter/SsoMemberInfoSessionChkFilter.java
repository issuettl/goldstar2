...
@Component("SsoMemberInfoSessionChkFilter")
@PropertySources(@PropertySource("classpath:/config/project.properties"))

public class SsoMemberInfoSessionChkFilter implements Filter {

	private Logger LOGGER = LoggerFactory.getLogger(SsoMemberInfoSessionChkFilter.class);
	...
	public SsoMemberInfoSessionChkFilter() {
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
		...
		if(servletPath.contains("ajax")||servletPath.contains("/api")||servletPath.contains("/gnb")|| servletPath.contains("/footer")||servletPath.contains("/breadCrumb")
			...
			if(servletPath.contains("/commonmodule/memberInfo") && isMobileApp && isAutoLogin){
				...
				//ACCESS_TOKEN 이 만료되었거나, REFRESH_TOKEN 만 존재하는 경우에 필터 수행
				if( ! ((getExpireTime(accessTokenA) <= System.currentTimeMillis() ||  NullUtil.isNone(accessTokenA)) && NullUtil.notNone(refreshTokenA)) ){
					filterChain.doFilter(servletRequest, servletResponse);
					return;
				}
		...
		// SSO 설정
		String ssoHost = configService.getString("ssoHost");
		String clientId = configService.getString("ssoClientId");
		String clientSecret = configService.getString("ssoClientSecret");
		String ssoLogin = configService.getString("ssoLogin");

		SSOClient ssoClient = new SSOClient(ssoHost);
		...
		if( accessTokenC!=null && !accessTokenC.equals("")&& !accessToken.equals(accessTokenC)){
			...
			String unifyIdC = getUnifyId(accessTokenC);
		...
		if(accessToken != null && !accessToken.equals("")){
			...
			try {
				...
				tokenInformation = ssoClient.validateToken(accessToken);
				TokenInformation.Status status = tokenInformation.getLogin().getStatus();
				Long expiresIn = tokenInformation.getExpires_in();

				if (status.equals(TokenInformation.Status.active)) {
					unifyId = getUnifyId(accessToken);
					// 토큰 유효 시간이 30분 이상
					if (expiresIn >= 1800L) {
					...
					// 토큰 갱신
						...
						writeFilterDebugLog(request, "L41 Request Token with RefreshToken : "+refreshToken);
						AccessTokenRequest tokenRequest = new AccessTokenRequest()
								.withClientKeyAndSecret(clientId, clientSecret)
								.withGrantType(AccessTokenRequest.GrantType.refresh_token)
								.withTokenType(AccessTokenRequest.TokenType.JWT).withRefreshToken(refreshToken);

						AccessTokenResponse accessTokenResponse;
						try {
							accessTokenResponse = ssoClient.requestToken(tokenRequest);
							...
							LOGGER.error("L42 Requesting token has failed.");
				...
				LOGGER.error("L42 Validating token has failed.");
				writeFilterDebugLog(request, String.format("L42 ValidateTokenException: %s", e.getMessage()));
			}
		}

		/** L5. MobileAPP 자동로그인 인 경우에  예외발생 등으로 로그인 되지 않은 경우 RefreshToken . **/
		if(isMobileApp && isAutoLogin && !isLoggedIn){
			...
			if(refreshToken != null && !refreshToken.equals("") ){
				...
				SSOClient ssoClient2 = new SSOClient(ssoHost);
				try {
					accessTokenResponse = ssoClient2.requestToken(tokenRequest);
					...
					LOGGER.error("L52 Requesting token has failed");
					writeFilterDebugLog(request, String.format("L52 RefreshTokenException : %s", e.getMessage()));

					/** L52 RefreshToken 비정상(network) 예외 발생시 최대 한번 더 시도 (SSO 정상예외는 refresh_token 문자열로 판단) **/
					if(!e.getMessage().contains("refresh_token")) {
						AccessTokenRequest tokenRequest2 = new AccessTokenRequest().withClientKeyAndSecret(clientId, clientSecret)
								.withGrantType(AccessTokenRequest.GrantType.refresh_token)
								.withTokenType(AccessTokenRequest.TokenType.JWT).withRefreshToken(refreshToken);

						AccessTokenResponse accessTokenResponse2 = null;
						SSOClient ssoClient3 = new SSOClient(ssoHost);
						try {
							accessTokenResponse2 = ssoClient3.requestToken(tokenRequest);

							accessToken = accessTokenResponse2.getAccess_token();
							refreshToken = accessTokenResponse2.getRefresh_token();
							unifyId = getUnifyId(accessToken);
							isLoggedIn = true;
							isAutoLoginViaMobileApp = true;
						} catch (Exception sube) {
							initOBSConf(request, response);
							LOGGER.error("L522 Requesting token has failed");
							writeFilterDebugLog(request, String.format("L522 RefreshTokenException : %s", sube.getMessage()));
		...
		if(isLoggedIn){
			...
			if(isMobileApp) {
				writeFilterDebugLog(request, String.format(
						"L61 Final Info - AccessToken: %s, RefreshToken: %s, UnifyId: %s, isLoggedIn: %s, isMobileApp: %s, isAutoLogin: %s, isTokenValidationException: %s, isTokenRefreshException: %s",
			...
			if(isMobileApp) {
				writeFilterDebugLog(request, String.format(
						"L62 Final Info - AccessToken: %s, RefreshToken: %s, UnifyId: %s, isLoggedIn: %s, isMobileApp: %s, isAutoLogin: %s, isTokenValidationException: %s, isTokenRefreshException: %s",
						accessToken, refreshToken, unifyId, isLoggedIn, isMobileApp, isAutoLogin, isTokenValidationException, isTokenRefreshException));
			...
			if (requiredLogin(request, requiredList, exceptionList)) {
				...
				String url = String.format("%s?state=%s", ssoLogin, state);
				response.sendRedirect(url);
	...
	public String getUnifyId(String access_token) {
		String unifyId = "";
		try {
			JWTClaimsSet jwtClaimsSet;
			jwtClaimsSet = JwtUtils.parseToken(access_token);
			JSONObject contexts = (JSONObject) jwtClaimsSet.getClaim("context");
			Map user = JsonUtils.convertValue(contexts.get("user"), Map.class);
			unifyId = (String) user.get("unifyId");

		} catch (Exception e) {
			// TODO Auto-generated catch block
			//e.printStackTrace();
			LOGGER.error("access_token validation has failed {} : {}", access_token, e.getMessage());
		}
		return unifyId;
	}

	public long getExpireTime(String access_token) {
		...
		try {
			...
			// TODO Auto-generated catch block
			//e.printStackTrace();
			LOGGER.error("access_token checking has failed {} : {}", access_token, e.getMessage());
		}
