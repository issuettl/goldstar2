...
<!-- popup start -->
<section id="mindQuestion" class="pop_mindTest modal">
    <div class="popBg">
        <div class="popContent">
            ...
            <!-- 질문 start -->
            <div id="mindQuestionZone" class="con_mindTest questionZone">
                ...
                <c:forEach items="${questions}" var="q" varStatus="qIndex">
                    <div id="<c:out value="mindQuestions${qIndex.count}"/>" class="mind_Q<c:out value="${qIndex.count}"/> mindTest_q mindQuestions" data-answers="<c:out value="mindAnswers-${q.sn}"/>"
                    ...
                    >
	                    ...
	                    <ul class="mindTest_list number<c:out value="${fn:length(q.answers)}"/>">
	                       <c:forEach items="${q.answers}" var="a" varStatus="aIndex">
	                        <li>
	                            <label for="<c:out value="mindAnswer-${a.sn}"/>" class="test_radio">
	                                <input type="radio" name="<c:out value="mindAnswers-${q.sn}"/>" id="<c:out value="mindAnswer-${a.sn}"/>" value="<c:out value="${a.sn}"/>">
	                    ...
	                    <div class="btn_wrap">
                            ...
                            <button onClick="showQuestion('<c:out value="mindQuestions${qIndex.count + 1}"/>', 'mind');" class="full btn_primary btn_pop_s">다음</button>
                            </c:if>
	                        <c:if test="${qIndex.count > 1}">
                            <button onClick="showQuestion('<c:out value="mindQuestions${qIndex.count - 1}"/>', 'mind');" class="half btn_gray btn_pop_s">뒤로가기</button>
                            <button onClick="showQuestion('<c:out value="mindQuestions${qIndex.count + 1}"/>', 'mind');" class="half btn_primary btn_pop_s">다음</button>
                            </c:if>
	                    </div>
	                </div>
                </c:forEach>
            ...
            <!-- 질문 end -->
            <!-- 결과 A1 start -->
            <div id="mindResult" class="con_mindResult questionResult">
                 <div class="mindReslut_about tac">
                    <figure class="mind01_result1"><img id="mindResultImage" src="<c:url value="/u/images/popup/mind_result_11.png"/>" alt=""></figure>
                    <div class="mindReslut_text">
                        <h3 id="mindResultTitle"></h3>
                        <p id="mindResultRecommend"></p>
                    </div>
                </div>
                <!-- 답변지 qr -->
                <div class="qr_wrap">
