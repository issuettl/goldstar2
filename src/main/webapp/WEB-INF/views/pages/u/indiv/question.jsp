...
<!-- popup start -->
<section id="indivQuestion" class="pop_mindTest modal">
    <div class="popBg">
        <div class="popContent">
            ...
            <!-- 질문 start -->
            <div id="indivQuestionZone" class="con_mindTest questionZone">
                ...
                <c:forEach items="${questions}" var="q" varStatus="qIndex">
                    <div id="<c:out value="indivQuestions${qIndex.count}"/>" class="mind_Q<c:out value="${qIndex.count}"/> mindTest_q indivQuestions" data-answers="<c:out value="indivAnswers-${q.sn}"/>"
                    ...
                    >
	                    ...
	                    <ul class="mindTest_list number<c:out value="${fn:length(q.answers)}"/>">
	                       <c:forEach items="${q.answers}" var="a" varStatus="aIndex">
	                        <li>
	                            <label for="<c:out value="indivAnswer-${a.sn}"/>" class="test_radio">
	                                <input type="radio" name="<c:out value="indivAnswers-${q.sn}"/>" id="<c:out value="indivAnswer-${a.sn}"/>" value="<c:out value="${a.sn}"/>">
	                    ...
	                    <div class="btn_wrap">
                            ...
                            <button onClick="showQuestion('<c:out value="indivQuestions${qIndex.count + 1}"/>', 'indiv');" class="full btn_primary btn_pop_s">다음</button>
                            </c:if>
	                        <c:if test="${qIndex.count > 1}">
                            <button onClick="showQuestion('<c:out value="indivQuestions${qIndex.count - 1}"/>', 'indiv');" class="half btn_gray btn_pop_s">뒤로가기</button>
	                        <button onClick="showQuestion('<c:out value="indivQuestions${qIndex.count + 1}"/>', 'indiv');" class="half btn_primary btn_pop_s">다음</button>
                            </c:if>
	                    </div>
	                </div>
                </c:forEach>
            ...
            <!-- 질문 end -->
            <!-- 결과 A1 start -->
            <div id="indivResult" class="con_mindResult questionResult">
                <div class="mindReslut_about tac">
                    <figure class="mind01_result1"><img id="indivResultImage" src="<c:url value="/u/images/popup/mind_result_11.png"/>" alt=""></figure>
                    <div class="indivReslut_text">
                        <h3 id="indivResultTitle"></h3>
                        <p id="indivResultRecommend"></p>
                    </div>
                </div>
                <!-- 답변지 qr -->
                <div class="qr_wrap">
